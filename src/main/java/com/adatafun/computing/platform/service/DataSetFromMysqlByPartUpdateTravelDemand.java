...
public class DataSetFromMysqlByPartUpdateTravelDemand {
    ...
    public static void main(String[] args) throws Exception {
        ...
        sql_user.add("select id, name_str from tb_user where name_str in (select name from tb_order_limousine " +
                "where name is not null and service_date > CURDATE());");
        sql_user.add("select id, name_str from tb_user where name_str in (select name from tb_order_vvip " +
                "where name is not null and service_date > CURDATE());");
        ...
        sql_product.add("select name, service_date, TIMESTAMPDIFF(MINUTE,CURDATE(),service_date) as remaining_time " +
                "from tb_order_limousine where name is not null and service_date > CURDATE();");
        sql_product.add("select name, service_date, TIMESTAMPDIFF(MINUTE,CURDATE(),service_date) as remaining_time " +
                "from tb_order_vvip where name is not null and service_date > CURDATE();");
        sql_product.add("select phone, service_date, TIMESTAMPDIFF(MINUTE,CURDATE(),service_date) as remaining_time " +
                "from tb_order_loungeserv where name is not null and service_date > CURDATE();");
            ...
            DataSet<Tuple2<String, String>> input = input_user.join(input_product).where(1).equalTo(0)
                    .with(new JoinFunction<Tuple2<Long, String>, Tuple3<String, String, String>, Tuple2<String, String>>() {
                        ...
                        public void flatMap(Tuple2<String, String> value, Collector<Tuple2<String, String>> out) throws ParseException {
                            ...
                            Tuple2<String, String> tuple2 = new Tuple2<>();
                            Long remainingTime = Long.valueOf(value.f1);
        ...
        if (dataUnion != null) {
            ...
            DataSet<Map> dataTransfer = dataUnion.groupBy(0, 1).first(1).groupBy(0)
                ...
                public void flatMap(Tuple2<String, String> value, Collector<Map> out) throws ParseException{
                    ...
                    map.put("recentTravel", value.f1);
                    if (value.f1.equals("非近期出行")) {
                        map.put("recentTravel_flag", "非近期出行");
                    } else {
                        map.put("recentTravel_flag", "近期出行");
                    }
                    out.collect(map);
